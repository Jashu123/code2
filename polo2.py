...
class FteTest:
    ...
    def check_vssr_compliance(self) -> List[str]:
        ...
        if self.pool == "Feature":
            if not self.has_argument('--vssr_enable'):
                issues.append("Pool=Feature but missing --vssr_enable argument")
        elif self.pool == "IOStress":
            if not self.has_argument('--stress_test'):
                issues.append("Pool=IOStress but missing --stress_test argument")
...
def parse_args():
    ...
    parser.add_argument(
        "--vssr_check", 
        action="store_true",
        help="Check Pool=Feature for --vssr_enable and Pool=IOStress for --stress_test"
    )
    parser.add_argument(
        "--full_check",
        action="store_true", 
...
# VSSR compliance check - reports missing --vssr_enable (Feature pool) or --stress_test (IOStress pool)  
...
def main():
    ...
    if args.full_check:
        ...
        colored_print("\n🔍 IRIS COMPLIANCE CHECK", Colors.BLUE, bold=True)
        ...
        if total_iris_issues == 0:
            colored_print("✅ All tests are IRIS compliant!", Colors.GREEN, bold=True)
        else:
            colored_print(f"❌ Found {total_iris_issues} IRIS issue(s) across {tests_with_iris_issues} test(s)", Colors.RED, bold=True)
        
        colored_print("\n🔍 VSSR COMPLIANCE CHECK", Colors.BLUE, bold=True)
        colored_print("-" * 30, Colors.BLUE)
        ...
        if total_vssr_issues == 0:
            colored_print(f"✅ All {relevant_vssr_tests} relevant tests are VSSR compliant!", Colors.GREEN, bold=True)
        else:
            colored_print(f"❌ Found {total_vssr_issues} VSSR issue(s) across {tests_with_vssr_issues} test(s)", Colors.RED, bold=True)
        ...
        if total_iris_issues > 0 or total_vssr_issues > 0:
            colored_print("\n⚠️  SKIPPING ORIGINAL VALIDATION", Colors.YELLOW, bold=True)
        ...
        colored_print("\n✅ All compliance checks passed! Proceeding with original validation...", Colors.GREEN, bold=True)
    ...
    # Handle IRIS check mode
        ...
        if total_issues == 0:
            colored_print("\n✅ All tests are IRIS compliant!", Colors.GREEN, bold=True)
        else:
            colored_print(f"❌ Found {total_issues} IRIS compliance issue(s) across {tests_with_issues} test(s)", Colors.RED, bold=True)
    ...
    # Handle VSSR check mode
        ...
        if total_issues == 0:
            colored_print(f"\n✅ All {relevant_tests} relevant tests are VSSR compliant!", Colors.GREEN, bold=True)
        else:
            colored_print(f"❌ Found {total_issues} VSSR compliance issue(s) across {tests_with_issues} test(s)", Colors.RED, bold=True)
