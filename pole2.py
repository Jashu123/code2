...
class CsvParser:
    ...
    def parse_iter(self) -> Generator[Tuple[int, "FteTest"], None, None]:
        ...
        # First pass to count total rows for percentage calculation
        with self.test_pool_path.open("r", encoding="utf-8") as test_pool_reader:
            total_rows = sum(1 for _ in test_pool_reader) - 1  # Subtract 1 for header
        ...
        with self.test_pool_path.open("r", encoding="utf-8") as test_pool_reader:
            ...
            for line_number, row in enumerate(test_pool_dict_reader, start=1):
                ...
                if total_rows > 0:
                    ...
                    print(f"\rValidating: {current_row}/{total_rows} ({percentage:.1f}%)", end='', flush=True)
                
                yield line_number, test
        
        # Print newline when parsing is complete
        print()
...
class CheckTestFunctionName(TestRuleHandler):
    ...
    def __call__(self, test: FteTest, test_index: int) -> None:
        ...
        # Check if --test_function_name argument exists in base_arguments
        ...
        for i, arg in enumerate(test.base_arguments):
            if arg.startswith("--test_function_name"):
                if "=" in arg:
                    ...
                    # If no '=' found, the value might be in the next argument
