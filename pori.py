...
class FteTest:
    def __init__(self, **kwargs) -> None:
        ...
        self.customer: List[str] = kwargs["customer"]  # Changed to List[str]
...
class CsvParser:
    def __init__(self, test_pool_path: Path):
        ...
        self.add_column("Customer", type_space_seperated_list, "customer")  # Changed to use type_space_seperated_list
    ...
    def modify_csv_for_iris(self):
        ...
        # Create backup
        backup_path = self.test_pool_path.with_suffix(f".backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        self.test_pool_path.rename(backup_path)
        ...
        with backup_path.open("r", encoding="utf-8") as input_file:
