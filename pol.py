...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="rollback",
        action="store_true",
        help="Rollback CSV file to the latest backup",
    )
    parser.add_argument(
        "--list-backups",
        dest="list_backups",
        action="store_true",
        help="List available backups for the CSV file",
    )
    parser.add_argument(
        "--clean-backups",
        dest="clean_backups",
        type=int,
        metavar="DAYS",
        help="Clean backup files older than specified days (default: 7)",
    )
    parser.add_argument(
        "--debug",
...
python test_pool_verifier.py --rollback -p test_pool.csv
python test_pool_verifier.py --list-backups -p test_pool.csv
python test_pool_verifier.py --clean-backups 7
"""
...
def main():
    ...
    if args_dict.get("rollback"):
        success = rollback_from_backup(test_pool_path)
        exit(0 if success else 1)

    # Handle listing backups
    if args_dict.get("list_backups"):
        backups = list_backups(test_pool_path)
        if backups:
            print(f"Available backups for {test_pool_path.name}:")
            for i, backup in enumerate(backups, 1):
                backup_time = datetime.fromtimestamp(backup.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')
                print(f"  {i}. {backup.name} (created: {backup_time})")
        else:
            print(f"No backups found for {test_pool_path.name}")
        exit(0)

    # Handle cleaning old backups
    if args_dict.get("clean_backups") is not None:
        days = args_dict.get("clean_backups")
        if days < 0:
            days = 7  # Default to 7 days if negative value provided
        cleaned_count = clean_old_backups(days)
        print(f"Cleaned {cleaned_count} backup files older than {days} days")
        exit(0)
