...
class OcpSmartAvailableSpare0(TestInstance):
    ...
    def get_bb_desc_list_for_no_die_retire(self, available_spare, non_gbb_in_usr_block):
        ...
        remain_bb = self.retired_block_count_threshold % self.lun_cnt
        lun_gbb_dict = {i: 0 for i in range(self.lun_cnt)}
        for lun_id in range(self.lun_cnt):
            if remain_bb > 0:
                lun_gbb_dict[lun_id] = self.retired_block_count_threshold // self.lun_cnt - lun_non_gbb_dict[lun_id] + 1
                remain_bb -= 1
            else:
                lun_gbb_dict[lun_id] = self.retired_block_count_threshold // self.lun_cnt - lun_non_gbb_dict[lun_id]
