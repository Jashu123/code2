...
class PCIeErrorMonitor:
    ...
    def run_pcie_stress_test(self, duration=60):
        ...
        try:
            result = subprocess.run(['lspci', '-xxx'], capture_output=True, check=True)
            ...
            self.logger.warning("lspci command not found")
        
        # If we can't access config space, try alternative checks
        if not config_accessible:
            try:
                # Check if PCIe directory exists
                pcie_dir = Path('/sys/bus/pci/devices')
                if pcie_dir.exists():
                    device_count = len(list(pcie_dir.glob('*')))
                    self.logger.info(f"Found {device_count} PCIe devices in sysfs")
                else:
                    self.logger.warning("PCIe sysfs directory not found")
            except Exception as e:
                self.logger.error(f"Error checking PCIe devices: {e}")
